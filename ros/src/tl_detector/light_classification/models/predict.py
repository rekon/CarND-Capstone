...
if __name__ == "__main__":
    ...
    # print(result*100, '%')
	for filepath in glob.glob('./sim_images/*.jpg'):
	
		image = cv2.imread(filepath)
		color = [ 'No Traffic Light', 'Traffic Light: Red', 'Traffic Light: Yellow','Traffic Light: Green' ]
		image_class = predict_camera_image(image)
		if 0 <= image_class <= 2:
			print('The predicted image',filepath,' class is -',color[image_class])
		else:
